#include<stdio.h>
#include<stdlib.h>
#include<errno.h>
#include<string.h>
#include<sys/types.h>
#include<sys/socket.h>
#include <arpa/inet.h>
#include<math.h>

#define LEN 1025

char re[] = "Message from server ";
char ty[] = "Type your message - ";
char cl[] = "Connection terminated\n";

char *convertToBits(char *inp,int padded)
{
	char *out = (char*)malloc(24*sizeof(char)); //convert 25 to 24
	int i,num,j,ind,k;
	for(i=0,k=0; i<3,k<(3-padded); i++,k++)
	{
		num = (int)inp[i];
		ind = i*8;
	    	for (j = 1 << 7; j > 0; j = j / 2)
	    	{
			if(num & j) out[ind++] = '1';
			else out[ind++] = '0';
		}
	}
	for(k=k*8;k<24;k++)
		out[k] = '0';
	return(out);
}

char* encodeThis(char *inp,int padded)
{
	int it,i,j,ind,num,k;
	for(i=0; i<3; i++)
		if(padded == i)
			it = 4 - i;
	char *bits = convertToBits(inp,padded);
	char *out = (char*)malloc(4*sizeof(char));
	
	for(i=0; i<it; i++)
	{
		ind = i*6;
		num = 0;
		for(j = ind+5 ; j>=ind; j--)
		{
			k = pow(2,ind+5-j);
			num += (bits[j]-'0')*k;
		}
		if(num>=0&&num<=25)
			out[i] = (char)(num + 65);
		if(num>=26&&num<=51)
			out[i] = (char)(num + 71);
		if(num>=52&&num<=61)
			out[i] = (char)(num-4);
		if(num==62)
			out[i] = '+';
		if(num==63)
			out[i] = '/';
	}
	while(it<4)
		out[it++] = '=';
		
	return(out);
}

char* encode(char *mg)
{
	int len = strlen(mg);
	int rem = len%3,p;
	int pad,zeroes;
	if(rem==1) pad = 2;
	if(rem==2) pad =1;
	if(rem==3) pad = 0;
	if(rem==0) p = 0;
	else p = 4;  
	int outlen = 4*(len-rem)/3 + p + 1;// extra 1 is for null character indicate the end of encoding

	char* output = (char*)malloc(outlen*sizeof(char));
	output[outlen] = '\0';
	char *en, arr[3];
	
	int i,j,count=0;
	for(i=0; i<len; i = i+3)
	{
		if(i+1==len)
		{
			arr[0] = mg[i];
			arr[1] = '=';
			arr[2] = '=';
			zeroes = 2;
		}
		else if(i+2==len)
		{
			arr[0] = mg[i];
			arr[1] = mg[i+1];
			arr[2] = '=';
			zeroes = 1;
		}
		else
		{
			arr[0] = mg[i];
			arr[1] = mg[i+1];
			arr[2] = mg[i+2];
			zeroes = 0;
		}
		en = encodeThis(arr,zeroes);
		for(j=0; j<4; j++)
			output[count*4+j] = en[j];
		count++;
	}
	
	return(output);
}

int convertToNumber(char *ptr)
{
	int ans=0,i = 0;
	int len = strlen(ptr);
	for(i=0; i<len; i++)
	{
		ans = ans*10 + (ptr[i] - '0');
	}
	return(ans);
}

void initialise(char *p)
{
	int i;
	for(i=0; i<LEN+1; i++)
	{
		p[i] = '\0';
	}
}

int main(int argc, char *argv[])
{
	char response[2], clmg[] = "Do you want to close the connection? - enter y or n and press enter\n";
	struct sockaddr_in server;
	int csocket = socket(AF_INET,SOCK_STREAM,0);
	if(csocket==-1)
	{
		perror("client socket");
		exit(-1);
	}
	server.sin_family = AF_INET;
	if(inet_aton(argv[1],&server.sin_addr)==0)
	{
		printf("error\n");
	}
	int pt = convertToNumber(argv[2]);
	server.sin_port = htons(pt);
	int len = sizeof(struct sockaddr_in);
	if(connect(csocket,(struct sockaddr *) &server, len)==-1)
	{
		perror("connect");
		printf("%d\n",errno);
		exit(-1);
	}
	char mg[LEN+1];// one extra is for appending null character to the mesage
	while(1)
	{
		initialise(mg); // intiialise the message wiht null characters
		write(1,clmg,strlen(clmg));
		read(0,response,2);
		if(response[0]=='y')
		{
			mg[0] = '2';
			char *ptr = encode(mg);
			write(csocket,ptr,strlen(ptr));
			write(1,cl,strlen(cl));
			break;
		}
		write(1,ty,strlen(ty));
		mg[0] = '1';
		int i=1;
		while(1)
		{
			read(0,response,1);
			if(response[0]=='\n'||i==LEN)// because mg[LEN-1] always contain the null character and this indiates that lenght of 	
				break;		     //message has reached 1024 byes.
			mg[i++] = response[0];
		}
		char *ptr = encode(mg);
		write(csocket,ptr,strlen(ptr));
		int st = read(csocket,mg,30);
		write(1,re,strlen(re));
		printf("%s:%d - ",argv[1],pt);
		fflush(stdout);
		write(1,mg,st);
		write(1,"\n",1);
	}
	//write(csocket,msg,strlen(msg));
	close(csocket);
	return 0;
}